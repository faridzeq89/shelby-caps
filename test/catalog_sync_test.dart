...
void main() {
  ...
  Future<Product> product(int catId, String name, int price,
      {int? wholesale}) async {
    final id = await db.into(db.products).insert(ProductsCompanion.insert(
        name: name,
        categoryId: catId,
        basePriceCents: price,
        wholesalePriceCents: Value(wholesale)));
  ...
  test('buildSnapshot arma productos (con mayoreo y umbral) y variantes', () async {
    final catId =
        await db.into(db.categories).insert(CategoriesCompanion.insert(name: 'Gorras'));
    final p = await product(catId, 'Shelby', 20000, wholesale: 15000);
    ...
    // El mayoreo viaja dentro del producto, más el umbral global en el snapshot.
    expect(snap.products.single['wholesale_price_cents'], 15000);
    expect(snap.wholesaleThreshold, 7);
  ...
  test('descarta variantes de productos que no van en el snapshot', () async {
    final catId =
    ...
    // Variante de un producto "fantasma" (id 999) no incluido.
    final ghost = Variant(
        id: 50,
        productId: 999,
        sku: 'GHOST',
    ...
    // El producto sin mayoreo publica null (no rompe la tienda).
    expect(snap.products.single['wholesale_price_cents'], isNull);
  });
}
